...
| Phone areas | seed-only | Úrad pre reguláciu elektronických komunikácií a poštových služieb numbering data | `https://www.teleoff.gov.sk/` | pending | Source/licence verification pending. | Pending verification. | Attribute the telecom regulator numbering data if reuse is approved; exact wording pending. | official source page/file URL pending | pending | Locate and retain the official Excel workbook URL, then verify reuse, caching, transformation, redistribution, commercial-use, and attribution terms. |
